    ...
    <section className="bg-white rounded-xl overflow-hidden shadow-md border border-outline-variant mb-8">
      <div className="relative h-64 w-full">
        ...
        <div className="absolute bottom-4 right-4">
          <span className="bg-primary text-white px-3 py-1 rounded-full font-bold text-xs shadow-md uppercase tracking-wider">{category}</span>
        </div>
      </div>
      ...
      <div className="p-8">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
          <div className="flex-1">
            <h1 className="font-headline text-3xl font-extrabold text-primary mb-2">{title}</h1>
            ...
            <div className="flex flex-wrap gap-4">
              <div className="flex items-center gap-2 bg-surface-container px-4 py-2 rounded-lg border border-outline-variant">
                ...
                <span className="font-bold text-sm">{questionsCount} Questions</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 w-full md:w-auto">
            ...
            <button onClick={() => setIsEditOpen(true)} className="bg-surface-container-high text-on-surface font-bold px-8 py-3 rounded-xl border-2 border-outline hover:bg-surface-variant transition-all flex items-center justify-center gap-2">
      ...
      <EditQuizModal quizId={quizId} initialTitle={title} initialDesc={description} isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} onSuccess={onEditSuccess} />
    </section>
  );
}